...
ModuleInstance::ModuleInstance(Environment* env, bool is_host)
    : memory_index(kInvalidIndex),
      table_index(kInvalidIndex),
      is_host(is_host),
      env(env) {}

ModuleInstance::ModuleInstance(Environment* env, string_view name, bool is_host)
    : name(name.to_string()),
      memory_index(kInvalidIndex),
      table_index(kInvalidIndex),
      is_host(is_host),
...
Result Thread::TableCopy(const uint8_t** pc) {
  ...
  if (ok || !copy_backward) {
    ...
    if (size > 0) {
      ...
      memmove(data + dst, data + src, size * sizeof(table->entries[0]));
    }
  }
  TRAP_IF(!ok, TableAccessOutOfBounds);
  return Result::Ok;
...
Result Thread::RefFunc(const uint8_t** pc) {
  ...
  return Push(Ref{ExternalKind::Func, index});
}
  ...
  for (int i = 0; i < num_instructions; ++i) {
    ...
    switch (opcode) {
      ...
      case Opcode::CallIndirect: {
        ...
        assert(ref.kind == ExternalKind::Func);
        TRAP_IF(ref.index == kInvalidIndex, UninitializedTableElement);
...
#undef ATOMIC_RMW
    ...
    case Opcode::V8X16Shuffle: {
      ...
      case Opcode::RefNull:
        CHECK_TRAP(Push(Ref{ExternalKind::Func, kInvalidIndex}));
        break;
