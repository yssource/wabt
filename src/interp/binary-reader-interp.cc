...
wabt::Result BinaryReaderInterp::OnInitExprRefNull(Index index) {
  ...
  init_expr_value_.set_ref({ExternalKind::Func, kInvalidIndex});
  return wabt::Result::Ok;
}
...
wabt::Result BinaryReaderInterp::OnElemSegmentElemExpr_RefNull(
  ...
  elem_segment_->elems.push_back({ExternalKind::Func, kInvalidIndex});
  return wabt::Result::Ok;
}
...
wabt::Result BinaryReaderInterp::OnElemSegmentElemExpr_RefFunc(
  ...
  if (segment_is_passive_) {
    elem_segment_->elems.push_back({ExternalKind::Func, func_index});
  } else {
    elem_segment_info_->src.push_back({ExternalKind::Func, func_index});
  }
  return wabt::Result::Ok;
}
