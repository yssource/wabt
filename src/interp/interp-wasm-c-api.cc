...
#ifndef NDEBUG
#define TRACE(str, ...) printf("CAPI: " str, ##__VA_ARGS__)
#else
#define TRACE(...)
#endif
...
enum class RefType { EXTERN, MODULE, INSTANCE, TRAP, HOST };

struct wasm_ref_t {
  wasm_ref_t(RefType kind) : kind(kind) {}
  wasm_ref_t(const wasm_ref_t& other) : kind(other.kind), host_info(other.host_info), finalizer(other.finalizer) {}
...
struct wasm_extern_ref_t : wasm_ref_t {
  wasm_extern_ref_t(ExternalKind kind, Index index)
      : wasm_ref_t(RefType::EXTERN), kind(kind), index(index) {}

  wasm_extern_ref_t(const wasm_extern_ref_t& other)
      : wasm_ref_t(other), kind(other.kind), index(other.index) {}

  wasm_ref_t* Copy() const override {
    return new wasm_extern_ref_t(*this);
  }
...
struct wasm_foreign_ref_t : wasm_ref_t {
  wasm_foreign_ref_t() : wasm_ref_t(RefType::HOST) {}
  wasm_foreign_ref_t(const wasm_foreign_ref_t& other) : wasm_ref_t(other) {}
...
struct wasm_instance_t : wasm_ref_t {
  wasm_instance_t(wasm_store_t* store, interp::DefinedModule* module)
      : wasm_ref_t(RefType::INSTANCE),
        ptr(std::make_shared<WasmInstance>(store, module)) {}

  wasm_instance_t(std::shared_ptr<WasmInstance> ptr)
      : wasm_ref_t(RefType::INSTANCE), ptr(ptr) {}

  wasm_instance_t(const wasm_instance_t& other) : wasm_ref_t(other), ptr(other.ptr) {}
...
struct wasm_module_t : wasm_ref_t {
  wasm_module_t(wasm_store_t* store, const wasm_byte_vec_t* in, ModuleMetadata* metadata)
      : wasm_ref_t(RefType::MODULE), store(store), metadata(metadata) {
...
struct wasm_extern_t : wasm_extern_ref_t {
  ...
  interp::Environment* Env() const {
    return instance.get()->store->env;
  }
...
struct wasm_trap_t : wasm_ref_t {
  wasm_trap_t(const wasm_message_t* msg) : wasm_ref_t(RefType::TRAP) {
...
static TypedValue ToWabtValue(const wasm_val_t& value) {
  ...
  switch (value.kind) {
    ...
    case WASM_FUNCREF: {
      ...
      if (ref->kind != RefType::EXTERN) {
        printf("unexpected reftype in ToWabtValue: %d\n", ref->kind);
        WABT_UNREACHABLE;
      }
      wasm_extern_t* ext = static_cast<wasm_extern_t*>(ref);
      out.set_ref(Ref{ext->kind, ext->index});
      break;
    }
    default:
...
static wasm_val_t FromWabtValue(std::shared_ptr<WasmInstance> instance,
  ...
  switch (value.type) {
    ...
    case Type::Funcref:
      ...
      if (value.get_ref().index == kInvalidIndex)
  ...
  for (size_t i = 0; i < num_exports; i++) {
    ...
    switch (exp.kind) {
      case ExternalKind::Func: {
        ...
        auto* type = new wasm_functype_t(*env->GetFuncSignature(func->sig_index));
        out->data[i] = new wasm_exporttype_t{name, type};
        break;
      }
      case ExternalKind::Global: {
        auto* g = env->GetGlobal(exp.index);
        auto* type = new wasm_globaltype_t({FromWabtType(g->type)}, g->mutable_);
        out->data[i] = new wasm_exporttype_t{name, type};
        break;
      }
      case ExternalKind::Table: {
        auto* t = env->GetTable(exp.index);
        auto* type = new wasm_tabletype_t({FromWabtType(t->elem_type)}, FromWabtLimits(t->limits));
        out->data[i] = new wasm_exporttype_t{name, type};
        break;
      }
      case ExternalKind::Memory: {
...
bool wasm_table_set(wasm_table_t* t, wasm_table_size_t index, wasm_ref_t* ref) {
  ...
  table->entries[index].index = ext ? ext->index : kInvalidIndex;
  return true;
}
...
bool wasm_table_grow(wasm_table_t* t,
  ...
  Ref init_ref{ExternalKind::Func, kInvalidIndex};
  if (ext) {
    init_ref.index = ext->index;
  }
